...
Borrowed from flask-peewee, some handy flask/peewee utilities.
...
from api.compat import text_type
...
def get_dictionary_from_model(model, fields=None, exclude=None):
    ...
    for field_name in curr_fields:
        ...
        if isinstance(
            field_obj,
            ForeignKeyField) and field_data and field_obj.rel_model in fields:
            rel_obj = getattr(model, field_name)
            data[field_name] = get_dictionary_from_model(rel_obj, fields,
                                                         exclude)
        else:
...
def path_to_models(model, path):
    ...
    if next in model._meta.rel:
        ...
        raise AttributeError('%s has no related field named "%s"' %
                             (model, next))
    if path:
