...
Borrowed from flask-peewee, plan to add additional querying options and
also make it a more standard REST interface.
...
from api.compat import reduce
from api.filters import make_field_tree
from api.serializers import Deserializer
from api.serializers import Serializer
from api.utils import PaginatedQuery
...
class APIKeyAuthentication(Authentication):
    ...
    def get_key(self, k, s):
        try:
            return self.get_query().where(self._key_field == k,
                                          self._secret_field == s).get()
...
class RestResource(object):
    ...
    # exclude certian fields from being exposed as filters.
    # For related fields use "__" notation, e.g. user__password
    filter_exclude = None
    filter_fields = None
    ...
    def __init__(self, rest_api, model, authentication, allowed_methods=None):
        ...
        self.allowed_methods = allowed_methods or ['GET', 'POST', 'PUT',
                                                   'DELETE']
        ...
        self._filter_fields = self.filter_fields \
            or self.model._meta.get_field_names()
        ...
        if self.include_resources:
            for field_name, resource in self.include_resources.items():
                ...
                self._filter_fields.extend(['%s__%s' % (
                    field_name, ff
                ) for ff in resource_obj._filter_fields])
                self._filter_exclude.extend(['%s__%s' % (
                    field_name, ff
    ...
    def process_query(self, query):
        ...
        for key in request.args:
            ...
            raw_filters[expr].append(
                (op, request.args.getlist(orig_key), negated))

        # do a breadth first search across the field tree created
        # by filter_fields, searching for matching keys in the
        # request parameters -- when found, filter the query accordingly
    ...
    def apply_filter(self, query, expr, op, arg_list, negated):
        ...
        if op == 'in':
            ...
            query_clauses = [constructor({query_expr: val})
                             for val in arg_list]
    ...
    def serialize_query(self, query):
        s = self.get_serializer()
        return [
            self.prepare_data(obj,
                              s.serialize_object(obj,
                                                 self._fields, self._exclude))
            for obj in query
        ]
    ...
    def get_urls(self):
        return (('/', self.require_method(self.api_list, ['GET', 'POST'])),
    ...
    def apply_ordering(self, query):
        ...
        if ordering:
            ...
            if column in self.model._meta.fields:
                ...
                query = query.order_by(field.asc() if not desc else
                                       field.desc())
    ...
    def save_related_objects(self, instance, data):
        for k, v in data.items():
            if k in self._resources and isinstance(v, dict):
                ...
                rel_obj, rel_models = rel_resource.deserialize_object(
...
class RestrictOwnerResource(RestResource):
    ...
    def save_object(self, instance, raw_data):
        ...
        return super(RestrictOwnerResource, self).save_object(instance,
                                                              raw_data)
...
class RestAPI(object):
    ...
    def register(self, model,
                 provider=RestResource,
                 auth=None,
                 allowed_methods=None):
        self._registry[model] = provider(self, model,
                                         auth or self.default_auth,
                                         allowed_methods)
        ...
        for provider in self._registry.values():
            ...
            for url, callback in provider.get_urls():
                ...
                self.blueprint.add_url_rule(
                    full_url, '%s_%s' % (api_name, callback.__name__),
